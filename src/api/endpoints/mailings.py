...
from src.core.dependencies import get_db, get_current_user, get_current_admin_user, get_notifier
...
async def create_mailing(
    ...
    if not current_user.get("is_expert"):
        raise HTTPException(status_code=403, detail="Только эксперты могут создавать рассылки.")

    expert_id = current_user["vk_id"]
    user_tariff = current_user.get("tariff_plan", "Начальный")

    try:
        new_mailing = await mailing_crud.create_mailing_request(db, expert_id, mailing_data, user_tariff)
...
@router.get("/admin/pending", response_model=List[mailing_schemas.MailingRead],
...
async def reject_mailing(
    ...
    mailing = await mailing_crud.update_mailing_status(db, mailing_id, "rejected", reason)
    if not mailing:
        raise HTTPException(status_code=404, detail="Рассылка не найдена.")
