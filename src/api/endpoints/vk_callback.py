...
async def vk_callback_handler(
    ...
    if event_type in ["donut_subscription_create", "donut_subscription_prolonged", "donut_subscription_price_changed"]:
...
async def handle_donut_active(data: dict, db: AsyncSession, notifier: Notifier, cache: redis.Redis, event_type: str):
    ...
    sub_res = await db.execute(select(DonutSubscription).filter(DonutSubscription.user_id == user_vk_id))
    ...
    if user.is_expert and user.allow_notifications:
        ...
        if event_type == "donut_subscription_create":
            msg = f"Спасибо за поддержку! Подписка VK Donut оформлена. Ваш уровень обновлен."
            await notifier.send_message(user_vk_id, msg)
        elif event_type == "donut_subscription_prolonged":
            msg = f"Подписка VK Donut продлена. Спасибо, что остаетесь с нами!"
            await notifier.send_message(user_vk_id, msg)


async def handle_donut_inactive(data: dict, db: AsyncSession, notifier: Notifier, cache: redis.Redis, event_type: str):
    user_vk_id = int(data.get("user_id"))
    ...
    if sub:
        sub.status = "expired" if event_type == "donut_subscription_expired" else "cancelled"
        sub.is_active = False
        await db.commit()
        await cache.delete(f"user_profile:{user_vk_id}")
        ...
        if user and user.allow_notifications:
             await notifier.send_message(user_vk_id, "Ваша подписка VK Donut истекла или была отменена.")
