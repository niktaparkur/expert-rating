...
from src.core.dependencies import get_db, get_notifier, get_current_admin_user # <-- Импортируем новую зависимость
...
async def get_top_experts(db: AsyncSession = Depends(get_db)):
    approved_experts_data = await expert_crud.get_experts_by_status(db=db, status="approved")

    response_users = []
    for user, profile, stats_dict, topics in approved_experts_data:
        user_data = expert_schemas.UserAdminRead.model_validate(user, from_attributes=True)
...
@router.get("/admin/pending", response_model=List[expert_schemas.ExpertRequestRead], dependencies=[Depends(get_current_admin_user)])
async def get_pending_experts(db: AsyncSession = Depends(get_db)):
    ...
    for user, profile in pending_experts:
        data = { "vk_id": user.vk_id, "first_name": user.first_name, "last_name": user.last_name, "photo_url": user.photo_url, "regalia": profile.regalia, "social_link": profile.social_link, "performance_link": profile.performance_link, "region": profile.region, "topics": [topic.topic_name for topic in profile.topics] }
        response.append(expert_schemas.ExpertRequestRead.model_validate(data))
    return response


@router.get("/admin/all_users", response_model=List[expert_schemas.UserAdminRead], dependencies=[Depends(get_current_admin_user)])
...
@router.post("/admin/{vk_id}/approve", status_code=200, dependencies=[Depends(get_current_admin_user)])
async def approve_expert(vk_id: int, db: AsyncSession = Depends(get_db), notifier: Notifier = Depends(get_notifier)):
...
@router.post("/admin/{vk_id}/reject", status_code=200, dependencies=[Depends(get_current_admin_user)])
async def reject_expert(vk_id: int, db: AsyncSession = Depends(get_db), notifier: Notifier = Depends(get_notifier)):
    profile = await expert_crud.set_expert_status(db=db, vk_id=vk_id, status="rejected")
    if not profile:
        raise HTTPException(status_code=404, detail="Expert profile not found")
    await notifier.send_moderation_result(vk_id=vk_id, approved=False, reason="Несоответствие требованиям")
    return {"status": "ok", "message": "Expert rejected"}


@router.post("/admin/{vk_id}/delete", status_code=200, dependencies=[Depends(get_current_admin_user)])
