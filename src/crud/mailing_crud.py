...
async def create_mailing_request(db: AsyncSession, expert_id: int, mailing_data: mailing_schemas.MailingCreate,
                                 user_tariff: str) -> Mailings:
    ...
    query = select(func.count(Mailings.id)).where(
        and_(
            ...
            func.DATE_FORMAT(Mailings.created_at, '%Y-%m') == func.DATE_FORMAT(func.now(), '%Y-%m')
        )
    )
    ...
    if current_mailings_count >= limit:
        raise ValueError(f"Превышен лимит рассылок для вашего тарифа ({current_mailings_count}/{limit} в этом месяце).")

    # 2. Создание объекта рассылки
    new_mailing = Mailings(
        expert_vk_id=expert_id,
...
async def get_pending_mailings(db: AsyncSession):
    query = select(Mailings).where(Mailings.status == "pending").order_by(Mailings.created_at.asc())
    result = await db.execute(query)
    return result.scalars().all()


async def update_mailing_status(db: AsyncSession, mailing_id: int, status: str, reason: str = None) -> Mailings | None:
...
async def get_trust_voters_for_expert(db: AsyncSession, expert_id: int) -> list[int]:
    ...
    query = select(Vote.voter_vk_id).where(
        and_(
            Vote.expert_vk_id == expert_id,
