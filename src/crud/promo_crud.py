...
async def validate_and_get_promo_code(
    ...
    if promo.activations_limit is not None and promo.activations_limit > 0:
        ...
        logger.debug(f"Total activations for '{promo.code}': {activations_count}. Limit: {promo.activations_limit}.")
        if activations_count >= promo.activations_limit:
            logger.warning(f"Promo code '{promo.code}' has reached its total activation limit.")
            return None
    ...
    user_activations_count = (await db.execute(user_count_query)).scalar_one_or_none() or 0
    logger.debug(
    ...
    if promo.user_activations_limit > 0 and user_activations_count >= promo.user_activations_limit:
        logger.warning(f"User '{user_vk_id}' has reached their activation limit for promo code '{promo.code}'.")
        return None

    logger.success(f"Promo code '{promo.code}' successfully validated for user '{user_vk_id}'.")
    return promo
...
async def create_promo_code(db: AsyncSession, promo_data: promo_schemas.PromoCodeCreate) -> PromoCode:
    ...
    existing_code_res = await db.execute(select(PromoCode).where(PromoCode.code == promo_data.code.upper()))
...
async def update_promo_code(db: AsyncSession, promo_id: int,
                            promo_data: promo_schemas.PromoCodeCreate) -> Optional[PromoCode]:
