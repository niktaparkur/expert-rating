...
export const EventInfoCard = ({event, showContextMenu, onMenuClick}: EventInfoCardProps) => {
    ...
    return (
        <SimpleCell
            ...
            after={
                showContextMenu && (
                    <div className="event-card-menu-container">
                        <IconButton onClick={() => onMenuClick(event)} aria-label="Действия с мероприятием">
                            <Icon28MoreVertical/>
                        </IconButton>
                        {event.has_tariff_warning && <div className="event-card-warning-dot"/>}
                    </div>
                )
            }
            subtitle={<Footnote>{`${timeString}, ${dateString}`}</Footnote>}
        >
