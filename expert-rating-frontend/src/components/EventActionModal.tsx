...
export const EventActionModal = ({ id, event, onClose, onShare, onDelete }: EventActionModalProps) => {
    if (!event) {
        return null;
    }
    ...
    const isFinished = new Date() > new Date(eventDate.getTime() + event.duration_minutes * 60000);

    return (
        <ModalPage id={id} onClose={onClose} header={<ModalPageHeader>Действия</ModalPageHeader>} settlingHeight={100}>
            <Group>
                <RichCell
                    before={
                        <Avatar size={48} style={{ background: 'var(--vkCom--color_background_accent_themed)' }}>
                ...
                {event.has_tariff_warning && (
                    <FormStatus title="Превышен лимит голосов" mode="error">
                        Чтобы все голоса были учтены в рейтинге, пожалуйста, повысьте ваш тариф.
                    </FormStatus>
                )}
            </Group>
