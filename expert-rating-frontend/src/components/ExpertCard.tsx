...
import { Icon20CheckCircleFillGreen, Icon20FavoriteCircleFillYellow } from '@vkontakte/icons';
...
const RatingBlock = ({ expertRating, communityRating, showCommunityRating }: { expertRating: number, communityRating: number, showCommunityRating: boolean }) => {
    return (
        <div className={`rating-block ${showCommunityRating ? 'double' : 'single'}`}>
            <div className="rating-segment">
                <div className="rating-value">{expertRating}</div>
                <Icon20CheckCircleFillGreen width={20} height={20} aria-label="Экспертный рейтинг" />
            </div>
            {showCommunityRating && (
                <>
                    ...
                    <div className="rating-segment">
                        ...
                        <Icon20FavoriteCircleFillYellow width={20} height={20} aria-label="Народный рейтинг" />
                    </div>
                </>
            )}
        </div>
...
export const ExpertCard = ({ expert, topPosition, onClick }: ExpertCardProps) => {
    if (!expert) {
        return null;
    }
    ...
    const topicsString = (expert.topics || []).map(topic => {
