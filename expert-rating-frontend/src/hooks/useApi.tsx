...
export const useApi = (): ApiHook => {
  const apiRequest = useCallback(
    async <T,>(
      ...
      try {
        ...
        console.warn(
          "API Request: Proceeding without token or with invalid one",
        );
      }
      ...
      const isCritical =
        (endpoint.includes("/vote") || endpoint.includes("/payment")) &&
        method === "POST";
      if (isCritical) {
  ...
  const apiDelete = useCallback(
    <T,>(endpoint: string, body: unknown = null) =>
      apiRequest<T>(endpoint, "DELETE", body),
    [apiRequest],
  );
