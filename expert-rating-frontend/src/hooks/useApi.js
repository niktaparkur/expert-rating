...
export const useApi = () => {
  ...
  const apiGet = useCallback(
    (endpoint) => apiRequest(endpoint, "GET"),
    [apiRequest],
  );
  const apiPost = useCallback(
    (endpoint, body) => apiRequest(endpoint, "POST", body),
    [apiRequest],
  );
  const apiPut = useCallback(
    (endpoint, body) => apiRequest(endpoint, "PUT", body),
    [apiRequest],
  );
  const apiDelete = useCallback(
    (endpoint) => apiRequest(endpoint, "DELETE"),
    [apiRequest],
  );
