...
import { Panel, PanelHeader, Group, Header, CardGrid, Spinner, Search, Placeholder } from '@vkontakte/vkui';
...
interface HomeProps { id: string; user: UserData | null; }
...
export const Home = ({ id, user }: HomeProps) => {
    ...
    const fetchExperts = useCallback(async (isNewSearch = false) => {
        if (isLoading) return;
        setIsLoading(true);
        ...
        try {
            ...
            const expertsWithPosition = data.items.map((expert: any, index: number) => ({
                ...expert,
                topPosition: (currentPage - 1) * PAGE_SIZE + index + 1
            }));

            setExperts(prev => isNewSearch ? expertsWithPosition : [...prev, ...expertsWithPosition]);
    ...
    }, [apiGet, isLoading, page, debouncedSearch]);
    ...
    const expertsToRender = experts;

    return (
        <Panel id={id}>
            <PanelHeader>Рейтинг Экспертов</PanelHeader>
            <Group>
                <Search value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Поиск по имени или направлению" />
            </Group>
            <Group header={<Header>Топ экспертов</Header>}>
                ...
                {!isLoading && expertsToRender.length === 0 && (
                    <Placeholder icon={<Icon56UsersOutline />} title="Эксперты не найдены">
