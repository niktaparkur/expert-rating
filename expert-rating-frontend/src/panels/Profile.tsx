...
import {
    Panel, PanelHeader, Group, ModalRoot, Search, Placeholder,
    Spinner, Alert, Header, Snackbar, PullToRefresh, PanelHeaderButton,
    useAdaptivity, ViewWidth, Button, Div, Text, ModalPage, ModalPageHeader, SimpleCell, Switch
} from '@vkontakte/vkui';
...
export const Profile = ({id, user, setCurrentUser, refetchUser, setPopout, setSnackbar}: ProfileProps) => {
    ...
    const performCancelVote = async (voteId: number, isExpertVote: boolean) => {
        ...
        try {
            ...
            setSnackbar(<Snackbar onClose={() => setSnackbar(null)} before={<Icon16Done/>}>Голос отменен</Snackbar>);
        } catch (error) {
            setSnackbar(<Snackbar onClose={() => setSnackbar(null)} before={<Icon16Cancel/>}>Ошибка</Snackbar>);
        } finally {
            setPopout(null);
        }
    };
    ...
    const {planned, archived} = useMemo(() => {
        const p: EventData[] = [], a: EventData[] = [];
        const nowTimestamp = new Date().getTime();
        myEvents.forEach(event => {
            const isoDateString = event.event_date.endsWith('Z') ? event.event_date : event.event_date + 'Z';
            const eventEndTimestamp = new Date(isoDateString).getTime() + event.duration_minutes * 60000;
            if (eventEndTimestamp < nowTimestamp) a.push(event); else p.push(event);
        });
        a.sort((evA, evB) => new Date(evB.event_date).getTime() - new Date(evA.event_date).getTime());
        return {planned: p, archived: a};
    }, [myEvents]);

    const groupedPlannedEvents = groupPlannedEvents(planned);
    const renderGroupedEvents = (group: EventData[], title: string) => (group.length > 0 &&
    ...
    const renderVotesContent = () => (
        <Group><Search value={searchQueryVotes} onChange={(e) => setSearchQueryVotes(e.target.value)}
                       placeholder="Поиск"/>{isLoadingVotes ?
            <Spinner/> : (filteredVotes.length > 0 ? (filteredVotes.map(vote => <VoteHistoryCard key={vote.id}
                                                                                                 vote={vote}
                                                                                                 onCancelVote={handleCancelVote}/>)) : (
                <Placeholder icon={<Icon56UsersOutline/>}>Вы еще не голосовали.</Placeholder>))} </Group>);
    const renderEventsContent = () => (<div style={{paddingBottom: '60px'}}><Group><Header
        indicator={<Text></Text>}>Запланированные</Header><Div><Button stretched size="l" mode="secondary"
                                                                       style={{width: '90%', margin: '0 auto'}}
                                                                       onClick={() => routeNavigator.push('/create-event')}>+
        Добавить Мероприятие</Button></Div>{isLoadingMyEvents ? <Spinner/> : (planned.length === 0 ? (
        <Placeholder>Нет запланированных
            мероприятий.</Placeholder>) : (<>{renderGroupedEvents(groupedPlannedEvents.today, 'Сегодня')}{renderGroupedEvents(groupedPlannedEvents.tomorrow, 'Завтра')}{renderGroupedEvents(groupedPlannedEvents.next7Days, 'Ближайшие 7 дней')}{renderGroupedEvents(groupedPlannedEvents.later, 'Позже')}</>))}
    </Group>{archived.length > 0 && (
        <Group><Header>Архив</Header>{archived.map(event => <EventInfoCard key={event.id} event={event} isArchived
                                                                           showContextMenu={false} onMenuClick={() => {
        }}/>)}</Group>)}</div>);

    const modal = (<ModalRoot activeModal={activeModal} onClose={() => setActiveModal(null)}><EventActionModal
        id="event-actions-modal" event={selectedEvent} onClose={() => setActiveModal(null)} onShare={handleShare}
        onDelete={handleDeleteEvent}/><ModalPage id="profile-settings-modal" onClose={() => setActiveModal(null)}
                                                 header={
                                                     <ModalPageHeader>Настройки</ModalPageHeader>}><Group><SimpleCell
        Component="label" disabled={!user?.is_expert}
        after={<Switch checked={user?.show_community_rating ?? true} onChange={handleSettingsChange}/>}>Показывать
        народный рейтинг</SimpleCell></Group></ModalPage></ModalRoot>);

    return (
        <Panel id={id}>
            {modal}
            <PanelHeader after={isDesktop && <PanelHeaderButton
                onClick={onRefresh}><Icon28RefreshOutline/></PanelHeaderButton>}>Аккаунт</PanelHeader>
            <PullToRefresh onRefresh={onRefresh} isFetching={isFetching}>
                <UserProfile user={user} onSettingsClick={() => setActiveModal('profile-settings-modal')}
                             onWithdraw={() => {
                             }} isWithdrawLoading={isWithdrawLoading} isExpert={user?.is_expert || false}
