...
export const Voting = ({ id, user, setPopout, setSnackbar, refetchUser }) => {
  ...
  const handleEventVoteSubmit = async (voteData) => {
    ...
    if (!voteData.vote_type) {
      setSnackbar(
        <Snackbar onClose={() => setSnackbar(null)}>
          Пожалуйста, выберите "Доверяю" или "Не доверяю".
        </Snackbar>,
      );
      return;
    }
    ...
    switch (eventData.status) {
      case "active":
        return (
          <>
            <Group
              header={<Header mode="secondary">Эксперт мероприятия</Header>}
            >
      ...
      case "not_started":
        ...
        return (
          <Placeholder
            ...
            header="Голосование еще не началось"
